...
export class CurationService {
  ...
  private static async assertCustomersFollowTrader(traderId: string, customerIds: string[]) {
    ...
    for (const row of links) {
      if (row.followingId === traderId) allowed.add(row.followerId);
      else if (row.followerId === traderId) allowed.add(row.followingId);
    }
    if (allowed.size !== unique.length) {
      throw new AppError(
        400,
        'You can only share with people connected to you (they follow you, or you follow them as a customer). Refresh the list and try again.',
      );
    }
  }
  ...
  static async getTraderCustomers(traderId: string) {
    ...
    const byId = new Map<string, { id: string; name: string; businessName?: string | null; role: string }>();

    for (const c of inbound) {
      if (!c.follower.isActive) continue;
