...
`MainActivity` calls `GarmentHubSharePlugin.cacheShareExtrasFromIntent(applicationContext, intent)` on `onCreate` / `onNewIntent` (before `super`) so listing identity can be copied into a static cache before the embedding replaces the intent. **Intent-only** resolution (for the early cache write) is **extra → `Intent.data` URIs → `Intent.EXTRA_SHORTCUT_ID`** (Samsung sometimes keeps the shortcut id when stripping custom extras and `data`). Full resolution when capturing/peeking adds **in-memory cache** last. Canonical `resolvedSource` strings are **`extra`**, **`uri`**, **`shortcutId`**, **`cache`**, **`none`**. Shortcut id `share_product_{sanitizedId}` is written to a small **SharedPreferences** map whenever shortcuts sync so the id maps back to the real `productId` / `productName`.
